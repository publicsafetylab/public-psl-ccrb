...
ccrb["Precinct"] = np.where(ccrb["Command"].str.endswith(" PCT"), ccrb["Command"].str.replace(" PCT",""), ccrb["Command"])
...
merged = pd.merge(merged, table, how='left', left_on=['Year', 'Precinct'], right_on=['YEAR', 'ADDR_PCT_CD']).drop(labels=['YEAR', 'ADDR_PCT_CD'], axis=1)
...
else: raise Exception("Mismatch between raw and process counts")
